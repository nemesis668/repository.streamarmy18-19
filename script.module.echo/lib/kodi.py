...
def get_path():
    if PY2: return addon.getAddonInfo('path').decode('utf-8')
    else: return addon.getAddonInfo('path')

def get_profile():
    if PY2: return addon.getAddonInfo('profile').decode('utf-8')
    else: return addon.getAddonInfo('profile')

def translate_path(path):
    if PY2: return translatePath(path).decode('utf-8')
    else: return translatePath(path)
if PY2:
...
def get_kodi_version():
    ...
    class KodiVersion(object):
        ...
        version = xbmc.getInfoLabel('System.BuildVersion').decode('utf-8') if PY2 else xbmc.getInfoLabel('System.BuildVersion')
        ...
        try: revision = revision.decode('utf-8') if PY2 else revision
        except: revision = u''
        try: tag = tag.decode('utf-8') if PY2 else tag
        except: tag = u''
