...
def menu():
    ...
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
              ...
              for i in r if i]
        r = [(urljoin(base_domain,i[0][0].attrs['href']), i[1][0].attrs['alt'], urljoin(base_domain,i[1][0].attrs['src'])) for i in r]
        if ( not r ):
...
def content(url,searched=False):
    ...
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
              ...
              for i in r if i]
        r = [(urljoin(base_domain,i[0][0].attrs['href']), i[1][0].attrs['alt'], urljoin(base_domain,i[1][0].attrs['src']), i[2][0].content if i[2] else 'Unknown') for i in r]
    ...
    for i in r:
        try:
            if PY2:
                ...
                name = kodi.sortX(i[1])
                name = name.title() + ' - [ %s ]' % kodi.sortX(i[3])
